(function ($) {
window.tildaBrowserLang = window.navigator.userLanguage || window.navigator.language; window.tildaBrowserLang = window.tildaBrowserLang.toUpperCase(); if (window.tildaBrowserLang.indexOf('RU') != -1) { window.tildaBrowserLang = 'RU' } else { window.tildaBrowserLang = 'EN' }
    window.tildaForm = { versionLib: '01.001', isRecaptchaScriptInit: !1, currentFormProccessing: !1, arMessages: { 'EN': { 'success': 'Thank you! Your data has been submitted.', 'successorder': 'Thank you! Order created. Please wait. We are going to the payment...' }, 'RU': { 'success': 'Спасибо! Данные успешно отправлены.', 'successorder': 'Спасибо! Заказ оформлен. Пожалуйста, подождите. Идет переход к оплате....' } }, arValidateErrors: { 'EN': { 'email': 'Please put a correct e-mail', 'url': 'Please put a correct URL', 'phone': 'Please put a correct phone number', 'number': 'Please put a correct number', 'date': 'Please put a correct date', 'time': 'Please put a correct time (HH:mm)', 'name': 'Please put a name', 'namerus': 'Please put a correct name (only cyrillic letters)', 'nameeng': 'Please put a correct name (only latin letters)', 'string': 'You put incorrect symbols. Only letters, numbers and punctuation symbols are allowed', 'req': 'Please fill out all required fields', 'reqfield': 'Required field', 'emptyfill': 'No one field is filled' }, 'RU': { 'email': 'Укажите, пожалуйста, корректный email', 'url': 'Укажите, пожалуйста, корректный URL', 'phone': 'Укажите, пожалуйста, корректный номер телефона', 'number': 'Укажите, пожалуйста, корректный номер', 'date': 'Укажите, пожалуйста, корректную дату', 'time': 'Укажите, пожалуйста, корректное время (ЧЧ:ММ)', 'name': 'Укажите, пожалуйста, имя', 'namerus': 'Укажите, пожалуйста, имя (только кириллица)', 'nameeng': 'Укажите, пожалуйста, имя (только латиница)', 'string': 'Вы написали некорректные символы. Разрешены только буквы, числа и знаки пунктуации', 'req': 'Пожалуйста, заполните все обязательные поля', 'reqfield': 'Обязательное поле', 'emptyfill': 'Ни одно поле не заполнено' } } }; $(document).ready(function () {
        ...
        window.tildaForm.validate = function ($jform) {
            var arError = []; var isEmptyValue = !0; $jform.find('.js-tilda-rule').each(function () {
                var req = $(this).data('tilda-req') || 0; var rule = $(this).data('tilda-rule') || 'none', regExp, str, domainpart; var error = {}; var val = $(this).val(); var valnospace = ''; error.obj = $(this); error.type = []; if (val && val.length > 0) { valnospace = val.replace(/\s/g, ''); val = val.trim() }
                ...
                if (req == 1 && ((val.length == 0 && valnospace.length == 0) || (($(this).attr('type') == 'checkbox' || $(this).attr('type') == 'radio') && $(this).closest('form').find('[name="' + $(this).attr('name') + '"]:checked').length == 0))) { error.type.push('req') } else {
                    switch (rule) {
                        case 'email': regExp = /^[a-zA-Zёа-яЁА-Я0-9_\.\-\+]{1,64}@[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéû0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéû0-9\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,10}$/; if (val.length > 0 && !val.match(regExp)) { error.type.push('email') }
                            break; case 'url': regExp = /^((https?|ftp):\/\/)?[a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9][a-zA-Zёа-яЁА-ЯЁёäöüÄÖÜßèéûşç0-9\.\-]{0,253}\.[a-zA-Zёа-яЁА-Я]{2,10}\/?$/; if (val.length > 0) {
            ...
            return arError
        }; window.tildaForm.hideErrors = function ($jform) { $jform.find('.js-errorbox-all').hide(); $jform.find('.js-rule-error').hide(); $jform.find('.js-error-rule-all').html(''); $jform.find('.js-successbox').hide(); $jform.find('.js-error-control-box .t-input-error').html(''); $jform.find('.js-error-control-box').removeClass('js-error-control-box'); $jform.removeClass('js-send-form-error'); $jform.removeClass('js-send-form-success') }; window.tildaForm.showErrors = function ($jform, arErrors) {
            ...
            var arLang = window.tildaForm.arValidateErrors[window.tildaBrowserLang] || {}; var $fieldgroup, isFieldErrorBoxExist, isShowCommonErrors, $errItem, sError = ''; for (var i = 0; i < arErrors.length; i++) {
            ...
            if (event.origin.indexOf('forms.tildacdn.com') != -1) {
                if (event.data == 'closeiframe') { $('#tildaformcaptchabox').remove(); $('#js-tildaspec-captcha').remove(); return }
            ...
            $('#tildaformcaptchabox').remove(); $('#js-tildaspec-captcha').remove(); $jform.append('<input type="hidden" name="tildaspec-tildacaptcha" id="js-tildaspec-captcha">'); var randomkey; try { randomkey = '' + new Date().getTime() + '=' + parseInt(Math.random() * 8) } catch (e) { randomkey = 'rnd=' + parseInt(Math.random() * 8) }
            ...
            $jform.find('.js-tilda-payment').remove(); var product, i, j, html = ''; window.tildaForm.arProductsForStat = []
            ...
            for (i = 0; i < iProductsCount; i = i + 1) {
                product = window.tcart.products[i]; arProduct = {}; optionlabel = ''; for (j in product) {
                    if (typeof (product[j]) != 'function') {
                        if (j == 'options') {
                            for (var o in product[j]) {
                                if (product[j][o].option) { html += '<input type="hidden" name="tildapayment[products][' + i + '][' + j + '][' + o + '][option]" value="' + product[j][o].option + '" class="js-tilda-payment">' }
                        ...
                        } else { html += '<input type="hidden" name="tildapayment[products][' + i + '][' + j + ']" value="' + product[j] + '" class="js-tilda-payment">' }
                    }
                }
            ...
            if (window.tcart.delivery && window.tcart.delivery.name) {
                html += '<input type="hidden" name="tildapayment[delivery][name]" value="' + window.tcart.delivery.name + '" class="js-tilda-payment">'; if (window.tcart.delivery && window.tcart.delivery.price) {
                    html += '<input type="hidden" name="tildapayment[delivery][price]" value="' + window.tcart.delivery.price + '" class="js-tilda-payment">'; arProduct = { name: window.tcart.delivery.name, price: window.tcart.delivery.price, quantity: 1, id: 'delivery' }
                    window.tildaForm.arProductsForStat.push(arProduct)
                }
            }
            $jform.append(html)
        ...
        window.tildaForm.payment = function ($jform, arNext) {
            var html = ''; if ($jform.data('formpaymentoff') == 'y') { window.tcart = { amount: 0, currency: '', system: '', products: [] }; window.tcart.system = 'none'; return }
            ...
            window.tcart = { amount: 0, currency: '', system: '', products: [] }; if (arNext.message && arNext.message > '' && $jform.find('.js-successbox').length > 0) { $jform.find('.js-successbox').html(arNext.message); $jform.find('.js-successbox').show() }
                window.location.href = arNext.value; return !0
            } else {
                if (arNext.type == 'form') {
                window.tcart = { amount: 0, currency: '', system: '', products: [] }; $('#js-tilda-payment-formid').remove(); var key = '', val = ''; html = '<form id="js-tilda-payment-formid" action="' + arNext.value.action + '" method="post"  style="position:absolue;opacity:0;width: 1px; height: 1px; left: -5000px;">'; arNext.value.action = ''; for (key in arNext.value) { val = arNext.value[key]; if (typeof (val) != 'function' && val > '') { html += "<input type='hidden' name='" + key + "' value='" + val + "' >" } }
                    html += '</form>'; $('body').append(html); $('#js-tilda-payment-formid').submit()
                } else { if (arNext.type == 'function') { var arArgs = arNext.value.args; eval(arNext.value.name + '($jform, arArgs);'); return !1 } else { window.tcart = { amount: 0, currency: '', system: '', products: [] }; if (arNext.type == 'text' && arNext.message && arNext.message > '' && $jform.find('.js-successbox').length > 0) { $jform.find('.js-successbox').html(arNext.message); $jform.find('.js-successbox').show() } } }
            }
        }; window.tildaForm.stripeLoad = function () { if (window.stripeapiiscalled !== !0) { var script = document.createElement("script"); script.type = "text/javascript"; script.src = "https://checkout.stripe.com/checkout.js"; document.body.appendChild(script); window.stripeapiiscalled = !0 } }; window.tildaForm.stripePay = function ($jform, arArgs) {
            if (window.stripeapiiscalled !== !0) { window.tildaForm.stripeLoad(); window.setTimeout(function () { window.tildaForm.stripePay($jform, arArgs) }, 200); return !1 }
            ...
            window.tildaForm.orderIdForStat = arArgs.invoiceid; window.stripehandler.open({
                name: companyname, image: companylogo, description: arArgs.description, amount: parseInt(parseFloat(arArgs.amount) * 100), currency: arArgs.currency, shippingAddress: arArgs.shipping == '1' ? !0 : !1, email: arArgs.email > '' ? arArgs.email : '', token: function (token, args) {
                    if (token && token.id) {
                        $.ajax({
                            type: "POST", url: 'https://forms.tildacdn.com/payment/stripe/', data: { projectid: arArgs.projectid, invoiceid: arArgs.invoiceid, token: token.id, email: token.email, currency: arArgs.currency, amount: parseInt(parseFloat(arArgs.amount) * 100) }, dataType: "json", success: function (json) {
                                var virtPage = '/tilda/' + $jform.attr('id') + '/payment/'; var virtTitle = 'Pay order in form ' + $jform.attr('id'); var virtPrice = arArgs.amount; var virtProduct = arArgs.description; if (window.Tilda && typeof Tilda.sendEventToStatistics == 'function') {
                                    var currency = $('#allrecords').data('tilda-currency'); if (!currency) { $('#allrecords').data('tilda-currency', arArgs.currency) }
                                ...
                                $jform.data('successmessage', ''); var successcallback = $jform.data('success-callback'); window.tcart = { amount: 0, currency: '', system: '', products: [] }; window.tildaForm.successEnd($jform, arArgs.successurl, successcallback); $jform.trigger('tildaform:aftersuccess')
                            }, fail: function () { }, timeout: 15 * 1000
                        })
                    }
                }
            })
        }; window.tildaForm.cloudpaymentLoad = function () { if (window.cloudpaymentsapiiscalled !== !0) { var script = document.createElement("script"); script.type = "text/javascript"; script.src = "https://widget.cloudpayments.ru/bundles/cloudpayments"; document.body.appendChild(script); window.cloudpaymentsapiiscalled = !0 } }; window.tildaForm.cloudpaymentPay = function ($jform, arArgs) {
            if (window.cloudpaymentsapiiscalled !== !0) { window.tildaForm.cloudpaymentLoad(); window.setTimeout(function () { window.tildaForm.cloudpaymentPay($jform, arArgs) }, 200); return !1 }
            ...
            $jpopup.data('old-style', $jpopup.attr('style')); $jpopup.attr('style', 'z-index:auto'); window.tildaForm.orderIdForStat = arArgs.invoiceid; window.cloudpaymentshandler.charge({ publicId: arArgs.publicId, description: arArgs.description, amount: parseFloat(arArgs.amount), currency: currency, accountId: arArgs.accountId, invoiceId: arArgs.invoiceId, requireEmail: arArgs.requireEmail == !0 ? !0 : !1, email: arArgs.email, data: arData }, function (options) {
                ...
                $jform.data('successmessage', ''); if (arArgs.failureurl > '') { window.location.href = arArgs.failureurl } else { $jpopup.find('.t706__cartwin-products').show(); $jpopup.find('.t706__cartwin-prodamount-wrap').show(); $jpopup.find('.t706__form-bottom-text').show(); $jform.find('.t-form__inputsbox').show() }
            }); return !1
            ...
            $jform.find('input[type=text]:visible').val(''); $jform.find('textarea:visible').html(''); $jform.find('textarea:visible').val(''); $jform.data('tildaformresult', { tranid: "0", orderid: "0" }); $jform.trigger('tildaform:aftersuccess')
                ...
                if (arArgs && arArgs.html > '') {
                    $('#allrecords').append(arArgs.html); $('.t-banktransfer .t-popup__close').off('click'); $('.t-banktransfer .t-popup__close').click(function () {
                        ...
                        $jbankform.off('submit'); $jbankform.find('.t-submit').off('click'); $jbankform.find('.t-submit').off('dblclick'); $jbankform.submit(function (e) {
                            e.preventDefault(); arErrors = window.tildaForm.validate($jbankform); if (arErrors && arErrors.length > 0) { window.tildaForm.showErrors($jbankform, arErrors); return !1 }
                            $.ajax({
                                type: "POST", url: 'https://forms.tildacdn.com/payment/banktransfer/', data: $jbankform.serialize(), dataType: "json", success: function (json) {
            ...
            $jform.find('input[type=text]:visible').val(''); $jform.find('textarea:visible').html(''); $jform.find('textarea:visible').val(''); $jform.data('tildaformresult', { tranid: "0", orderid: "0" })
            ...
            if (formtype == 2 || (!formtype && formskey > '')) {
                var $inputElem; $inputElem = $jform.find('input[name=tildaspec-cookie]'); if (!$inputElem || $inputElem.length == 0) { $jform.append('<input type="hidden" name="tildaspec-cookie" value="">'); $inputElem = $jform.find('input[name=tildaspec-cookie]') }
                ...
                if (formskey > '') {
                    $inputElem = $jform.find('input[name=tildaspec-formskey]'); if (!$inputElem || $inputElem.length == 0) { $jform.append('<input type="hidden" name="tildaspec-formskey" value="">'); $inputElem = $jform.find('input[name=tildaspec-formskey]') }
                ...
                $jform.find('.js-form-spec-comments').val(''); $formurl = 'https://script.google.com/macros/s/AKfycbx4386dPQIO-M0OHGUbsSMcjUbF1xoQ3ibEaZud3K11A_n3CX0/exec'; $.ajax({
                    type: "GET", url: $formurl, data: $jform.serialize(), dataType: "json", success: function (json) {
                        var successurl = $jform.data('success-url'); var successcallback = $jform.data('success-callback'); var formsendedcallback = $jform.data('formsended-callback'); btnformsubmit.removeClass('t-btn_sending'); btnformsubmit.data('form-sending-status', '0'); btnformsubmit.data('submitform', ''); if (json && json.error) {
                            ...
                            $allError.html(json.error).show(); $errBox.show(); $jform.addClass('js-send-form-error'); $jform.trigger('tildaform:aftererror')
                        } else {
                            ...
                            var virtTitle = 'Send data from form ' + $jform.attr('id'); var virtPrice = 0; var virtProduct = ''; if (window.Tilda && typeof Tilda.sendEventToStatistics == 'function') {
                                if ($jform.find('.js-tilda-price').length > 0) { virtPrice = $jform.find('.js-tilda-price').val(); if (parseFloat(virtPrice) > 0) { virtTitle = 'Order ' + formres.orderid } }
                                Tilda.sendEventToStatistics(virtPage, virtTitle, virtProduct, virtPrice); if (window.dataLayer) { window.dataLayer.push({ 'event': 'submit_' + $jform.attr('id') }) }
                            } else {
                ...
                if ($jform.data('is-formajax') == 'y') {
                    $.ajax({
                        type: "POST", url: $jform.attr('action'), data: $jform.serialize(), dataType: "text", success: function (html) {
                                ...
                                if (html.substring(0, 1) == '{') {
                                    ...
                                    if (json && json.message) { if (json.message != 'OK') { $jform.find('.js-successbox').html(json.message) } } else {
                                        if (json && json.error) {
                                            ...
                                            $allError.html(json.error); $allError.show(); $errBox.show(); $jform.addClass('js-send-form-error'); $jform.trigger('tildaform:aftererror'); return !1
                                        }
                                    }
                                } else { $jform.find('.js-successbox').html(html) }
                            }
                            var virtPage = '/tilda/' + $jform.attr('id') + '/submitted/'; var virtTitle = 'Send data from form ' + $jform.attr('id'); if (window.Tilda && typeof Tilda.sendEventToStatistics == 'function') { window.Tilda.sendEventToStatistics(virtPage, virtTitle, '', 0) } else {
                ...
                } else { var attraction = $jform.attr('action'); if (attraction.indexOf('forms.tildacdn.com') == -1) { btnformsubmit.data('form-sending-status', '3'); $jform.submit(); return !0 } else { return !1 } }
            }
        }; $('.js-tilda-captcha').each(function () {
            if ($(this).attr('data-tilda-captchakey') > '') {
                if (window.tildaForm.isRecaptchaScriptInit === !1) { window.tildaForm.isRecaptchaScriptInit = !0; $('head').append('<script src="https://www.google.com/recaptcha/api.js?render=explicit"' + ' async defer><' + '/script>'); $('head').append('<style type="text/css">.js-send-form-success .grecaptcha-badge {display: none;}</style>') }
                var idform = $(this).attr('id'); if ($('#' + idform + 'recaptcha').length == 0) { $(this).append('<div id="' + idform + 'recaptcha" class="g-recaptcha" data-sitekey="' + $(this).attr('data-tilda-captchakey') + '" data-callback="window.tildaForm.captchaCallback" data-size="invisible"></div>') }
            } else { $(this).removeClass('js-tilda-captcha') }
        }); $('.js-tilda-mask').each(function () { var mask = $(this).data('tilda-mask'); var maskplaceholder = $(this).data('tilda-mask-holder'); if (mask) { if (maskplaceholder && maskplaceholder > '') { $(this).mask('' + mask, { placeholder: '' + maskplaceholder }) } else { $(this).mask('' + mask) } } }); $('.js-tilda-rule').on('focus', function () { var str = $(this).attr('placeholder'); if (str && str.length > 0) { $(this).data('placeholder', str); $(this).attr('placeholder', '') } }); $('.js-tilda-rule').on('blur', function () { var str = $(this).data('placeholder'); if (str > '') { $(this).attr('placeholder', str); $(this).data('placeholder', '') } }); window.validateForm = function ($jform) { return window.tildaForm.validate($jform) }
        ...
        $('.r').find('.js-form-procces').each(function () { try { var formtype = $(this).data('formactiontype'); if (formtype == 2) { $(this).attr('action', '#') } } catch (e) { console.log(e) } }); $('.r').off('submit', '.js-form-proccess'); $('.r').on('submit', '.js-form-proccess', function (e) { var btnformsubmit = $(this).find('[type=submit]'); var btnstatus = btnformsubmit.data('form-sending-status'); if (btnstatus && btnstatus == 3) { btnformsubmit.data('form-sending-status', ''); return !0 } else { $(this).find('[type=submit]').trigger('click'); return !1 } }); $('.r').on('dblclick', '.js-form-proccess [type=submit]', function (e) { e.preventDefault(); return !1 }); $('.r').off('click', '.js-form-proccess [type=submit]'); $('.r').on('click', '.js-form-proccess [type=submit]', function (event) {
            ...
            btnformsubmit.addClass('t-btn_sending'); btnformsubmit.data('form-sending-status', '1'); btnformsubmit.data('submitform', $activeForm); window.tildaForm.hideErrors($activeForm); arErrors = window.tildaForm.validate($activeForm); if (window.tildaForm.showErrors($activeForm, arErrors)) { btnformsubmit.removeClass('t-btn_sending'); btnformsubmit.data('form-sending-status', '0'); btnformsubmit.data('submitform', ''); return !1 } else {
                var formtype = $activeForm.data('formactiontype'); var formskey = $('#allrecords').data('tilda-formskey'); if ($activeForm.find('.js-formaction-services').length == 0 && formtype != 1 && formskey == '') {
            ...
            var TILDAPAGE_URL = window.location.href, TILDAPAGE_QUERY = '', TILDAPAGE_UTM = ''; if (TILDAPAGE_URL.toLowerCase().indexOf('utm_') !== -1) {
                TILDAPAGE_URL = TILDAPAGE_URL.toLowerCase(); TILDAPAGE_QUERY = TILDAPAGE_URL.split('?'); TILDAPAGE_QUERY = TILDAPAGE_QUERY[1]; if (typeof (TILDAPAGE_QUERY) == 'string') {
                    var arPair, i, arParams = TILDAPAGE_QUERY.split('&'); for (i in arParams) { if (typeof (arParams[i]) != 'function') { arPair = arParams[i].split('='); if (arPair[0].substring(0, 4) == 'utm_') { TILDAPAGE_UTM = TILDAPAGE_UTM + arParams[i] + '|||' } } }
...
})(jQuery); (function (factory) { if (typeof define === 'function' && define.amd) { define(['jquery'], factory) } else { if (typeof exports === 'object') { factory(require('jquery')) } else { factory(jQuery) } } }(function ($) {
